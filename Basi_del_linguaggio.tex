...
\subsection{Only Once Headers | Header Guards | pragma once | ifndef}

\label{header_guards}

\textsf{\small \textbf{Definizione: } Se un file header viene incluso due volte, il compilatore processerà il suo contenuto due volte, il che risulterà in un errore. Per evitare questo c'è una procedura standard, chiamate \emph{Header Guards}, da scrivere all'interno del file di intestazione.}
...
\textsf{\small Può essere utile per aiutare il compilatore ad assicurarsi che non ci siano stati errori di spelling o di numero sbagliato di argomenti da passare.} 
...
\textsf{\small Inoltre, potrebbe essere utile per evitare di includere molteplici headers in un header file.} \\

\textsf{\small \textbf{Esempio}: } 

\textsf{\small Abbiamo l'header A, l'header B, l'header C, l'header D: } \\

\begin{itemize}
	\item \textsf{\small Se nell'header A includiamo l'header B, questo prende spazio/aumenta il tempo di build.}
	\item \textsf{\small Se poi, l'header C ha bisogno di certi oggetti della classe B, includendo l'header B; include, di conseguenza, anche l'header A, anche se non aveva bisogno dell'header A, ma solo dell'header B.}
	\item \textsf{\small Se poi abbiamo l'header D che ha bisogno dell'header C e lo include, di conseguenza si prenderà anche l'header A e l'header B, prendendo ulteriore spazio/aumenta il tempo di build.}
	\item \textsf{\small Quindi, essenzialmente creiamo una matriosca che occupa spazio/aumenta il tempo di build.}
	\item \textsf{\small Ora, questo non dovrebbe essere un grosso problema se si includono le \emph{header guards} a pag.\pageref{header_guards} che impediscono che uno stesso header file venga incluso più di una volta, quando avviene la fase di linking.}
	%\item \textsf{\small }
\end{itemize}

\textsf{\small Se invece al posto usiamo la \emph{Forward Declaration}, possiamo fare così: } 

\textsf{\small Se nell'header A abbiamo bisogno di una classe B, al posto di includere l'header B, potremmo usare la \emph{Forward Declaration} in questo modo: } \\
...
\begin{lstlisting}
	...
	class A {
		public:
			...
			int x;
			int y;
			class B* b; // Forward Declaration (al posto di fare \#include "B.h", mettiamo class B, indicando al compilatore che useremo una classe B, ma che non è importante, per ora, sapere come questa classe B è implementata)
	};
\end{lstlisting}

\textsf{\small Quindi, nel nostro header A, inseriamo la dichiarazione della classe B, ma per l'implementazione di questa bisognerà andare nel file di implementazione (.cpp) dove verrà effettivamente incluso l'header B e quindi la definizione della classe B sarà presente.} \\
...
\textsf{\small Lo stesso possiamo fare per gli altri due headers C e D.} \\
